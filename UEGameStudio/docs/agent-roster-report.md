...
| --- | --- | --- | --- |
| [总控编排专家](../agents/orchestration/orchestration-director.md) | `orchestration-director` | 新任务首次强制提问、98% 需求理解与用户确认门禁、任务拆分、专业路由、依赖编排、规划期澄清、状态治理、冲突升级与综合交付 | 需求确认后可读取和委派；不可编辑、执行命令或联网 |

总控编排专家不替代专业判断，也不直接实施项目变更。每个新任务的首次响应必须先提问，不得读取项目、规划、委派或实施；只有需求理解置信度达到 98%、需求摘要经用户明确确认，且规划期未知项得到澄清后，才可形成任务计划。它负责确认谁决定、谁实施、谁验证，以及每项工作何时可以进入下一阶段。
...
总控编排专家、游戏总设计师、技术总监、游戏制作人和游戏视听总监具有 `task: allow`，用于调用其他专业 Agent；它们均禁止编辑和命令执行，符合“决策与实施分离”的原则。
...
用户对需求摘要的纠正或实质追加会使旧确认失效，总控必须重置置信度和确认状态。规划期问题如果改变目标、范围、固定决策、授权或验收条件，也必须返回需求澄清门禁，不得沿用旧计划。
...
2. 以核心数据源、资产包所有权和可验证交付物确定 Agent 边界，不按职位名称无限拆分。
3. 引用资产不转移写入权；跨域修改通过契约交回唯一所有者。
4. `.uasset` 只能通过 UE Editor 或受控自动化修改，工具不可用时返回 `BLOCKED_TOOLING`。
5. 关键输入缺失时返回 `BLOCKED_INPUT`；草案不得冒充批准基线或完成状态。
6. 每个实施任务使用路径、Package、操作和外部工具白名单，并核对实际写入。
7. 逻辑资产组与可生产子 Asset ID 分离，每个子对象只有一个内容写入主责。
8. 继续保持“制定—实施—验证”分离，实施者不能批准自己的创意、合规、性能或 QA 门禁。
9. 新 Agent 继续采用英文 kebab-case ID、中文专业正文和 `mode: subagent`。
10. 所有能力严格止于本地开发和构建游戏包，不扩展到商店、认证、正式发布与 LiveOps。
