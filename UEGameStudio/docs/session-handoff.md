...
- **git**：分支 `master`，remote `origin`（`https://github.com/losemymind/UEGameStudio.git`），HEAD `794873a`，与 `origin/master` 同步。
- **工作区**：有未提交改动，未 commit / push（明细见下节）。
...
1. **漂移清理（使门禁转绿）**
   ...
   - 影响文件：`agents/orchestration/batch-g3-coordinator.md`（工作区删除，未提交）。
2. **`.opencode/` 移出版本控制**
...
- `M .gitignore`
- `M AGENTS.md`
- `D UEGameStudio/agents/orchestration/batch-g3-coordinator.md`
- `D .opencode/**`（259 条，暂存的出索引删除）
- 本文件与 `docs/agent-roster-report.md` 的同步修改

## 已知待办 / 潜在风险

- **未提交**：上述改动均未 commit / push，需用户明确指示后才能提交。`.opencode` 的 259 条暂存删除必须与其他改动分开提交，不要顺手一起提交。
...
注意：上一会话改动尚未提交，请勿擅自 commit / push；.opencode 的暂存删除须与其他改动分开处理。
```
