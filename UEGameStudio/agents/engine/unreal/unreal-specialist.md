...
### MCP / AI 工具集开发
- 暴露给 AI/Agent 的工具接口用 `UFUNCTION(meta=(AICallable))`；辅助类加 `MCP` 前缀避免引擎命名冲突
- 引擎版本差异先查 `VERSION.md`：某些版本的 API（如日志宏、错误脚本函数）可能未导出/被替换——自定义工具先查引擎 API 并模仿引擎写法，无现成才自创
- 工具源码放插件级模块（`<Toolset>/Classes/` + 模块注册 + `.Build.cs` 依赖）；引擎二进制发行版不可重编译时，扩展必须插件级自包含
- 移植分级：完美 → 等价 → 降级，非完美必须记录；所有工具代码过构建验证
- 审查清单：AICallable 存在、MCP 前缀、引擎 API 优先、构建+冒烟验证
