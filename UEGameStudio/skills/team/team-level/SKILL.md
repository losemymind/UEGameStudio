...
| 借口（会怎么说） | 反驳（为什么不对） |
|---|---|
| 「先摆布局再定视觉，效率高」 | 视觉目标是布局的输入约束，布局先于视觉会导致布局违背视觉方向而返工 |
| 「相邻区域没文档，自己脑补一个就行」 | 臆造内容会与真实相邻区域冲突，破坏跨关卡一致性 |
| 「关键路径用颜色区分就够了」 | 仅靠颜色标识违反色盲安全，色觉异常玩家无法导航 |

## Red Flags（违规信号）
- 布局先于视觉方向或与视觉地标矛盾
- 关键路径标识仅靠颜色
- 谜题认知负荷超过 3 个同时状态未警示
- 有 BLOCKING 无障碍问题仍推进到下一阶段

## Verification（证据化验证门）
- [ ] 步骤 1 视觉目标已作为布局输入传给 level-designer（附传递记录）
- [ ] 跨关卡依赖显式标记 UNRESOLVED 或已解决，无臆造内容
- [ ] 无障碍审查输出含 BLOCKING/RECOMMENDED 分级，BLOCKING 项有处理记录
- [ ] 关卡设计文档已写入 design/levels/[level-name].md 且含遭遇数、资产清单
