...
| 借口（会怎么说） | 反驳（为什么不对） |
|---|---|
| 「改动我看一眼就知道，不用 git 取上一版」 | 必须 git show HEAD 取上一版做概念性 diff，凭印象会漏掉那些影响架构的隐性变更。 |
| 「这些 ADR 明显都过期了，直接批量标 Superseded」 | 逐个 ADR 询问，不批量决定；批量标记会掩盖真正仍有效的决策。 |
| 「直接改 ADR 正文更快，省得留一堆备注」 | 非破坏性，绝不删除 ADR 内容，只追加 Superseded by 备注，保留可审计历史。 |

## Red Flags（违规信号）
- 未产出变更影响报告就更新 ADR 状态或写出文件。
- 直接改写或删除 ADR 正文内容。
- 只扫直接引用、未检查对其它 ADR 或系统的级联影响。
- 未经用户逐个确认就批量修改多个 ADR 的状态。

## Verification（证据化验证门）
- [ ] 变更摘要来自 git 上一版与当前版的概念性 diff，而非印象判断。
- [ ] 影响报告含每个受影响 ADR 的假设/现状/评估/建议动作，并标 ✅/⚠️/🔴 分类。
- [ ] 每个 ADR 的状态更新为追加 Superseded by 备注，原正文未被删除。
- [ ] 每个 ADR 都有用户逐个确认的处理选择（Superseded/Update/Keep/Skip）。
