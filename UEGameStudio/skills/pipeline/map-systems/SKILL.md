...
| 借口（会怎么说） | 反驳（为什么不对） |
|---|---|
| 「概念文档里没写战斗细节，先只列显式系统」 | 必须推断隐式系统（伤害/生命/状态/敌人 AI 等）并向用户解释为何需要，否则索引不完整。 |
| 「这个系统优先级低于 X，所以先做 X」 | 优先级理由要用技术必要性 + 玩家体验解释，不能只写纯技术依赖。 |
| 「清单我都核对过了，直接写索引」 | 枚举/依赖/优先级都必须展示给用户验证，写前要征得同意，未经批准不得写盘。 |

## Red Flags（违规信号）
- 未经用户批准就把整套系统清单写入 systems-index.md。
- 优先级理由只写"X 依赖 Y"而无对玩家体验的影响说明。
- 未检测循环依赖、瓶颈系统或叶子节点。
- 在本技能内越权去生成单系统 GDD（那是 /design-system 的职责）。

## Verification（证据化验证门）
- [ ] 枚举表同时含显式与隐式系统，且每个系统有名称/类别/一句描述/显式或隐式标注。
- [ ] 依赖图已按 Foundation→Core→Feature→Presentation→Polish 分层，循环依赖有解决方案。
- [ ] 每次写文件前有 "May I write to [filepath]?" 确认，且写索引前展示摘要并征得同意。
- [ ] 已更新 production/session-state/active.md，且选系统后交接到 /design-system 而非重复其工作流。
