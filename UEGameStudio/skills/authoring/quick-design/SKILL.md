...
| 借口（会怎么说） | 反驳（为什么不对） |
|---|---|
| 「这个功能虽然大，但写 quick spec 省时间」 | 超过一周/显著跨系统依赖的改动必须走 /design-system，硬塞会埋下未评审的架构隐患 |
| 「数值直接写进文档方便阅读」 | 所有值必须走 assets/data/*.json，硬编码会导致单一事实源失效、日后难调优 |
| 「顺手把 GDD 也改了，反正差不多」 | 改 GDD 需显式批准并展示 old vs new 差异，擅自改会污染正式设计文档 |

## Red Flags（违规信号）
- 把需要一周以上的功能或显著跨系统契约变更写进 quick spec
- 与既有 GDD 规则冲突却不引用原文对比
- 未征得同意就修改 GDD 文件
- Tuning 类去查代码而非 assets/data/ 数据文件

## Verification（证据化验证门）
- [ ] 改动类别（Tuning/Tweak/Addition/New Small System）已推断并经用户确认
- [ ] 规格已与既有 quick-specs 和 GDD 交叉检查，冲突处有原文对比
- [ ] 所有数值位于 assets/data/*.json，文档中无硬编码值
- [ ] 若规格标记需更新 GDD，已单独征得同意并展示 old vs new 文本
