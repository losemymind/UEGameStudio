---
name: setup-engine
description: 配置项目游戏引擎并锁定版本：在 AGENTS.md 中钉住引擎、写入技术偏好与专家路由，并在版本超出训练数据时通过 WebSearch 填充引擎参考文档。Use when 需要选定/切换引擎、锁定版本或升级引擎版本。
---
...
### 更新技术栈
1. 更新 AGENTS.md 的 Technology Stack。Godot 需先问语言（GDScript / C# / 两者）；Unity 用 C#；Unreal 用 C++ + Blueprint，构建系统为 Unreal Build Tool (UBT)，资产管线为 Unreal Content Pipeline。写前展示并征得同意。
...
### 更新导入、agent 指令与收尾
1. 更新 AGENTS.md 的引擎引用导入；为引擎专家 agent 补充"版本意识"小节；输出设置摘要。
...
- 输出：AGENTS.md 技术栈、`technical-preferences.md`、`docs/engine-reference/<engine>/` 参考文档。

## 约束
- 绝不猜引擎版本，必须联网核实或用户确认。
- 写 AGENTS.md/参考文档前必须展示并征得同意，不覆盖已有文档。
...
| 「我直接把 AGENTS.md 和参考文档改了，效率高」 | 约束「写前必须展示并征得同意，不覆盖已有文档」。 |
