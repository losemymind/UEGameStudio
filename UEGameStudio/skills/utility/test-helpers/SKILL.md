...
| 借口（会怎么说） | 反驳（为什么不对） |
|---|---|
| 「常量我按常见值填一个，GDD 回头对」 | 约束「边界与常量应追溯到 GDD 公式，不发明数值」，臆造会使断言与设计不符。 |
| 「已有 helper 我直接覆盖成新版」 | 约束「永不覆盖已有辅助文件」，可能含手写定制。 |
| 「通用模板最省事，不用管项目现有风格」 | 反例「生成与项目现有测试风格不符的通用模板」，必须采样匹配已有 setup/断言习惯。 |
| 「改动不大直接写 tests/ 下」 | 约束「写文件前必须确认」。 |

## Red Flags（违规信号）
- 常量/边界无 GDD 溯源来源。
- 覆盖已存在的 tests/helpers/ 文件。
- 未采样现有测试模式就生成通用模板。
- 未经确认写入 tests/ 下的文件。

## Verification（证据化验证门）
- [ ] 生成的边界/常量均能对应到 GDD 公式（有溯源注释或引用）。
- [ ] 已采样至多 5 个现有测试文件，风格与项目一致。
- [ ] 已有 helper 文件未被覆盖（有跳过 + 提示记录）。
- [ ] 写文件前有展示与确认记录。
