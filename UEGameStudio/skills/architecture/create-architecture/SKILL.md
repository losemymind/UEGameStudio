...
| 借口（会怎么说） | 反驳（为什么不对） |
|---|---|
| 「引擎没配置，凭训练数据也能写」 | 训练数据有 cutoff，post-cutoff 新增/废弃 API 会写错，必须交叉引用引擎文档 |
| 「HIGH 风险领域先跳过，最后再补」 | 高风险领域是决策关键，忽略会产出与引擎不符的架构蓝图 |
| 「把章节攒齐了一次性写更完整」 | 增量写每批准一节立即写，攒到最后会话崩溃即全丢 |
| 「技术需求基线漏一条 GDD 的隐含约束没关系」 | 每条 GDD 约束都映射到 ADR 覆盖，漏掉会产生未覆盖缺口 |
