...
| 借口（会怎么说） | 反驳（为什么不对） |
|---|---|
| 「跨文档太慢，我先按单 GDD 内部查一遍」 | 本技能的核心就是跨文档关系审查，单 GDD 内部检查是 /design-review 的职责 |
| 「两套规则冲突，我判断 A 对就按 A 写」 | 本技能不做设计决策，只并列呈现矛盾让用户裁决 |
| 「这个 GDD 的引用失效了，我先替它改掉」 | 评审只标记问题，改 GDD 是另一条流程，且每条问题须引用具体章节原文 |

## Red Flags（违规信号）
- 报告未引用任何具体 GDD 章节/原文，问题描述含糊
- 一致性与设计理论两条线串行执行，或子代理被要求自己重读文件
- systems-index 状态值带括号后缀（如 "Needs Revision (x)"），破坏精确匹配
- 未经批准直接写 gdd-cross-review 报告或改 systems-index

## Verification（证据化验证门）
- [ ] 每条问题是否引用具体 GDD、章节与原文
- [ ] 2a–2f 与 3a–3g 是否逐项覆盖，且两条线并行 spawn 完成
- [ ] 跨系统场景走查是否走完触发→激活→数据流→失败模式并标注 BLOCKER/WARNING/INFO 严重度
- [ ] 写报告 / 更新 systems-index 前是否有 AskUserQuestion 批准记录，状态值无括号后缀
